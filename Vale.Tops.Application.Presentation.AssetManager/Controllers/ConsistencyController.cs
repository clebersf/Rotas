...
namespace Vale.Tops.Application.Presentation.AssetManager.Controllers
{
    public class ConsistencyController : Controller
    {
        ...
        {
            var ieList = (from Consistency in this.ConsistencyWriteRead.All()
                          join Location in LocationWriteRead.All() on Consistency.Id equals Location.Id
                          select Location);
            ...
            ConsistencyViewModel model = new ConsistencyViewModel { Locations = ieList, Selecteds = ieSelecteds, Message = DateTime.Now.ToString() + " - " + "Lista acessada.", lines = ieList.Count() };
        ...
        [HttpPost]
        public ActionResult Index(ConsistencyViewModel model)
        {
            try
            {
                if (model.Id != 0)
                {
                    this.ConsistencyWriteRead.Delete(model.Id); 
                ...
                else
                {
                    ...
                    for (int i = 0; i < model.Selecteds.Count(); i++)
                    {
                        ...
                        {
                            this.ConsistencyWriteRead.Delete(model.Selecteds[i].id);
                            log.Info("Deletado cadastro do registro " + model.Id.ToString() + " realizado pelo usuário " + User.Identity.Name);
                            none = false;
                        }                        
                    }
        ...
        public ActionResult Create()
        {
            ...
            var remove = (from Location in this.LocationWriteRead.All()
                          join rem in this.ConsistencyWriteRead.All() on Location.Id equals rem.Id
                          select Location);

            return PartialView(new ConsistencyViewModel { Locations = ieList.Except(remove) });
        }

      
        [HttpPost]
        public ActionResult Create(ConsistencyViewModel model)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    this.ConsistencyWriteRead.Save(new Consistency { Id = model.Id});
