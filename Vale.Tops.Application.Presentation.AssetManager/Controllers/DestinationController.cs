    ...
    public class DestinationController : Controller
    {
        ...
        public ActionResult Index()
        {
            var ieList = (from Location in this.LocationWriteRead.All()
                          join Destination in this.DestinationWriteRead.All() on Location.Id equals Destination.Id
                          select Location);
            ...
            DestinationViewModel model = new DestinationViewModel { Locations = ieList, Selecteds = ieSelecteds, Message = DateTime.Now.ToString() + " - " + "Lista acessada.", lines = ieList.Count() };
        ...
        public ActionResult Index(DestinationViewModel model)
        {
            try
            {
                if (model.Id != 0)
                {
                    this.DestinationWriteRead.Delete(model.Id); 
                ...
                else
                {
                    ...
                    for (int i = 0; i < model.Selecteds.Count(); i++)
                    {
                        ...
                        {
                            this.DestinationWriteRead.Delete(model.Selecteds[i].id);
                            log.Info("Deletado cadastro do registro " + model.Id.ToString() + " realizado pelo usuário " + User.Identity.Name);
                            none = false;
                        }                        
                    }
        ...
        public ActionResult Create()
        {
            ...
            var remove = (from Location in this.LocationWriteRead.All()
                          join rem in this.DestinationWriteRead.All() on Location.Id equals rem.Id
                          select Location);

            return PartialView(new DestinationViewModel { Locations = ieList.Except(remove) });
        }
        ...
        public ActionResult Create(DestinationViewModel model)
        {
            try
            {
                ...
                {
                    this.DestinationWriteRead.Save(new Destination { Id = model.Id});
