    ...
    public class FlowController : Controller
    {
        ...
        public ActionResult Index()
        {
            var ieList = (from Location in this.LocationWriteRead.All()
                          join Flow in this.FlowWriteRead.All() on Location.Id equals Flow.Id
                          select Location);
            ...
            FlowViewModel model = new FlowViewModel { Locations = ieList, Selecteds = ieSelecteds, Message = DateTime.Now.ToString() + " - " + "Lista acessada.", lines = ieList.Count() };
        ...
        public ActionResult Index(FlowViewModel model)
        {
            try
            {
                if (model.Id != 0)
                {
                    this.FlowWriteRead.Delete(model.Id); 
                ...
                else
                {
                    ...
                    for (int i = 0; i < model.Selecteds.Count(); i++)
                    {
                        ...
                        {
                            this.FlowWriteRead.Delete(model.Selecteds[i].id);
                            log.Info("Deletado cadastro do registro " + model.Id.ToString() + " realizado pelo usuário " + User.Identity.Name);
                            none = false;
                        }                        
                    }
        ...
        public ActionResult Create()
        {
            var ieList = (from Location in this.LocationWriteRead.All()
                          select Location);
            return PartialView(new FlowViewModel { Locations = ieList });
        }
        ...
        {
            var ieList = (from Location in this.LocationWriteRead.All()
                          join rLocation in this.FlowWriteRead.All() on Location.Id equals rLocation.Id
                          select Location);
            List<SelectedInt> ieSelecteds = new List<SelectedInt>();
            for (int i = 0; i < ieList.Count(); i++)
            {
        ...
        public ActionResult Create(FlowViewModel model)
        {
            try
            {
                ...
                {
                    this.FlowWriteRead.Save(new Flow { Id = model.Id});
        ...
        // POST: Asset/Edit/5
        [HttpPost]
        public ActionResult Edit(FlowViewModel model)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    this.FlowWriteRead.Edit(new Flow { Id = model.Id});
