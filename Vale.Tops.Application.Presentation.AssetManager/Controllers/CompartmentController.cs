...
namespace Vale.Tops.Application.Presentation.AssetManager.Controllers
{
    public class CompartmentController : Controller
    {
        ...
        private ICompartmentWriteRead CompartmentWriteRead;
        ILog log = LogManager.GetLogger("Admin Cadastro Location");
        public CompartmentController ()
        {
            this.LocationWriteRead = new LocationWriteRead();
            this.CompartmentWriteRead = new CompartmentWriteRead();
        }
        ...
        public ActionResult Index()
        {
            var ieList = (from Location in this.LocationWriteRead.All()
                          join Compartment in this.CompartmentWriteRead.All() on Location.Id equals Compartment.Id
                          select Location);
            ...
            CompartmentViewModel model = new CompartmentViewModel { Locations = ieList, Selecteds = ieSelecteds, Message = DateTime.Now.ToString() + " - " + "Lista acessada.", lines = ieList.Count() };
        ...
        [HttpPost]
        public ActionResult Index(CompartmentViewModel model)
        {
            try
            {
                if (model.Id != 0)
                {
                    this.CompartmentWriteRead.Delete(model.Id); 
                ...
                else
                {
                    ...
                    for (int i = 0; i < model.Selecteds.Count(); i++)
                    {
                        ...
                        {
                            this.CompartmentWriteRead.Delete(model.Selecteds[i].id);
                            log.Info("Deletado cadastro do registro " + model.Id.ToString() + " realizado pelo usuário " + User.Identity.Name);
                            none = false;
                        }                        
                    }
        ...
        public ActionResult Create()
        {
            ...
            var remove = (from Location in this.LocationWriteRead.All()
                          join rem in this.CompartmentWriteRead.All() on Location.Id equals rem.Id
                          select Location);

            return PartialView(new CompartmentViewModel { Locations = ieList.Except(remove) });
        }


        [HttpPost]
        public ActionResult Create(CompartmentViewModel model)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    this.CompartmentWriteRead.Save(new Compartment { Id = model.Id});
