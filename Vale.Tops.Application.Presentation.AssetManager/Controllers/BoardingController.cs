    ...
    public class BoardingController : Controller
    {
        ...
        public ActionResult Index()
        {
            var ieList = (from Location in this.LocationWriteRead.All()
                          join Boarding in this.BoardingWriteRead.All() on Location.Id equals Boarding.Id
                          select Location);
            ...
            BoardingViewModel model = new BoardingViewModel { Locations = ieList, Selecteds = ieSelecteds, Message = DateTime.Now.ToString() + " - " + "Lista acessada.", lines = ieList.Count() };
        ...
        [HttpPost]
        public ActionResult Index(BoardingViewModel model)
        {
            try
            {
                if (model.Id != 0)
                {
                    this.BoardingWriteRead.Delete(model.Id); 
                ...
                else
                {
                    ...
                    for (int i = 0; i < model.Selecteds.Count(); i++)
                    {
                        ...
                        {
                            this.BoardingWriteRead.Delete(model.Selecteds[i].id);
                            log.Info("Deletado cadastro do registro " + model.Id.ToString() + " realizado pelo usuário " + User.Identity.Name);
                            none = false;
                        }                        
                    }
        ...
        public ActionResult Create()
        {
            ...
            var remove = (from Location in this.LocationWriteRead.All()
                          join rem in this.BoardingWriteRead.All() on Location.Id equals rem.Id
                          select Location);

            return PartialView(new BoardingViewModel { Locations = ieList.Except(remove) });
        }

        [HttpPost]
        public ActionResult Create(BoardingViewModel model)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    this.BoardingWriteRead.Save(new Boarding { Id = model.Id});
