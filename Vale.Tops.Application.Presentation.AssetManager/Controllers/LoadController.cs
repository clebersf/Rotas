    ...
    public class LoadController : Controller
    {
        ...
        public ActionResult Index()
        {
            var ieList = (from Location in this.LocationWriteRead.All()
                          join Load in this.LoadWriteRead.All() on Location.Id equals Load.Id
                          select Location);
            ...
            LoadViewModel model = new LoadViewModel { Locations = ieList, Selecteds = ieSelecteds, Message = DateTime.Now.ToString() + " - " + "Lista acessada.", lines = ieList.Count() };
        ...
        public ActionResult Index(LoadViewModel model)
        {
            try
            {
                if (model.Id != 0)
                {
                    this.LoadWriteRead.Delete(model.Id); 
                ...
                else
                {
                    ...
                    for (int i = 0; i < model.Selecteds.Count(); i++)
                    {
                        ...
                        {
                            this.LoadWriteRead.Delete(model.Selecteds[i].id);
                            log.Info("Deletado cadastro do registro " + model.Id.ToString() + " realizado pelo usuário " + User.Identity.Name);
                            none = false;
                        }                        
                    }
        ...
        public ActionResult Create()
        {
            var ieList = (from Location in this.LocationWriteRead.All()
                          select Location);
            return PartialView(new LoadViewModel { Locations = ieList });
        }
        ...
        {
            var ieList = (from Location in this.LocationWriteRead.All()
                          join rLocation in this.LoadWriteRead.All() on Location.Id equals rLocation.Id
                          select Location);
            List<SelectedInt> ieSelecteds = new List<SelectedInt>();
            for (int i = 0; i < ieList.Count(); i++)
            {
        ...
        [HttpPost]
        public ActionResult Create(LoadViewModel model)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    this.LoadWriteRead.Save(new Load { Id = model.Id});
        ...
        // POST: Asset/Edit/5
        [HttpPost]
        public ActionResult Edit(LoadViewModel model)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    this.LoadWriteRead.Edit(new Load { Id = model.Id});
