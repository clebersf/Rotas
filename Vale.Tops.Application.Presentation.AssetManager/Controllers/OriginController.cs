    ...
    public class OriginController : Controller
    {
        ...
        public ActionResult Index()
        {
            var ieList = (from Location in this.LocationWriteRead.All()
                          join Origin in this.OriginWriteRead.All() on Location.Id equals Origin.Id
                          select Location);
            ...
            OriginViewModel model = new OriginViewModel { Locations = ieList, Selecteds = ieSelecteds, Message = DateTime.Now.ToString() + " - " + "Lista acessada.", lines = ieList.Count() };
        ...
        [HttpPost]
        public ActionResult Index(OriginViewModel model)
        {
            try
            {
                if (model.Id != 0)
        ...
        public ActionResult Create()
        {
            ...
            ieList.AddRange(from Location in this.LocationWriteRead.All()
                          select Location);

            var remove = (from Location in this.LocationWriteRead.All()
                          join rem in this.OriginWriteRead.All() on Location.Id equals rem.Id
                          select Location);

            return PartialView(new OriginViewModel { Locations = ieList.Except(remove) });
        }
        ...
        {
            var ieList = (from Location in this.LocationWriteRead.All()
                          join rLocation in this.OriginWriteRead.All() on Location.Id equals rLocation.Id
                          select Location);
            List<SelectedInt> ieSelecteds = new List<SelectedInt>();
            for (int i = 0; i < ieList.Count(); i++)
            {
        ...
        [HttpPost]
        public ActionResult Create(OriginViewModel model)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    this.OriginWriteRead.Save(new Origin { Id = model.Id});
        ...
        // POST: Asset/Edit/5
        [HttpPost]
        public ActionResult Edit(OriginViewModel model)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    this.OriginWriteRead.Edit(new Origin { Id = model.Id});
