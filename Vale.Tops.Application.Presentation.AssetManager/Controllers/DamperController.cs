    ...
    public class DamperController : Controller
    {
        ...
        public ActionResult Index()
        {
            var ieList = (from Location in this.LocationWriteRead.All()
                          join Damper in this.DamperWriteRead.All() on Location.Id equals Damper.Id
                          select Location);
            ...
            DamperViewModel model = new DamperViewModel { Locations = ieList, Selecteds = ieSelecteds, Message = DateTime.Now.ToString() + " - " + "Lista acessada.", lines = ieList.Count() };
        ...
        [HttpPost]
        public ActionResult Index(DamperViewModel model)
        {
            try
            {
                if (model.Id != 0)
                {
                    this.DamperWriteRead.Delete(model.Id); 
                ...
                else
                {
                    ...
                    for (int i = 0; i < model.Selecteds.Count(); i++)
                    {
                        ...
                        {
                            this.DamperWriteRead.Delete(model.Selecteds[i].id);
                            log.Info("Deletado cadastro do registro " + model.Id.ToString() + " realizado pelo usuário " + User.Identity.Name);
                            none = false;
                        }                        
                    }
        ...
        {
            var remove_1 = (from feed in this.DamperWriteRead.All()
            ...
            var remove_2 = (from dump in this.DamperWriteRead.All()
                            join loc in this.LocationWriteRead.All() on dump.Id equals loc.Id
                            join parent in this.DamperWriteRead.All() on loc.ParentId equals parent.Id
                            select parent);
            var Damper = this.DamperWriteRead.All().Except(remove_2);
            var add_1 = (from Location in this.LocationWriteRead.All()
                         join parent in Damper on Location.ParentId equals parent.Id
                         select Location);
            ...
            return PartialView(new DamperViewModel { Locations = ieList });
        }

        [HttpPost]
        public ActionResult Create(DamperViewModel model)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    this.DamperWriteRead.Save(new Damper { Id = model.Id});
