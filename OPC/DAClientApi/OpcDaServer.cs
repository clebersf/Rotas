...
namespace Siemens.Opc.Da
{
    ...
    {
        readonly int ReadWriteGroup = new Random().Next(1,10000);
        ...
        public void Connect(string url, int rate, int id)
        {
            lock (this)
            {
                if (!m_connected)
                {
                    try
                    {
                        ...
                        m_server.AddGroup(
                            "ReadWriteDefault"+ id.ToString(), //name
                            1,                  //active
                            rate,                  //requestedUpdateRate
                            id,     //ClientGroupHandle
