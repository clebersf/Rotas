...
using Vale.Tops.Integration.OpcDaDriver.Wpf.Models;
using Vale.Tops.Integration.OpcDaDriver.Wpf.Util;
using Vale.Tops.Domain;
using Vale.Tops.Integration.Infrastructure.DataBase.Repository.Source.PS_MSCS_SQL.AssetManager;

namespace Vale.Tops.Integration.OpcDaDriver.Wpf
{
    public partial class MainWindow : Window
    {
        private OpcDaPlcRunner _runner;
        public System.Collections.ObjectModel.ObservableCollection<UiTagRow> Tags { get; } = new System.Collections.ObjectModel.ObservableCollection<UiTagRow>();
        public MainWindow()
        {
            InitializeComponent();
            DataContext = this;
            _runner = new OpcDaPlcRunner(Log);
            _runner.OnOpcConfig += cfg => DispatcherEx.UI(() => { TxtProgId.Text = cfg.ProgId; TxtHost.Text = cfg.Host; TxtTopic.Text = cfg.Topic; TxtRate.Text = cfg.UpdateRate.ToString(); });
            _runner.OnTagsLoaded += rows => DispatcherEx.UI(() => { Tags.Clear(); foreach (var r in rows.OrderBy(x => x.TagName)) Tags.Add(new UiTagRow { TagId = r.TagId, TagName = r.TagName }); GridTags.ItemsSource = Tags; });
            _runner.OnValue += (tagId, value, ts) => { var row = Tags.FirstOrDefault(x => x.TagId == tagId); if (row != null) DispatcherEx.UI(() => { row.CurrentValue = value; row.LastTimestamp = ts.ToLocalTime(); }); };
            _runner.OnStatus += s => DispatcherEx.UI(() => TxtStatus.Text = s);
        }
        private async void BtnStart_Click(object s, RoutedEventArgs e) { BtnStart.IsEnabled = false; try { await _runner.StartAsync(); BtnStop.IsEnabled = true; Log("Started."); } catch (Exception ex) { Log("ERRO ao iniciar: " + ex.Message); BtnStart.IsEnabled = true; } }
        private async void BtnStop_Click(object s, RoutedEventArgs e) { BtnStop.IsEnabled = false; try { await _runner.StopAsync(); BtnStart.IsEnabled = true; Log("Stopped."); } catch (Exception ex) { Log("ERRO ao parar: " + ex.Message); BtnStop.IsEnabled = true; } }
        private async void BtnWrite_Click(object sender, RoutedEventArgs e)
        {
            if ((sender as FrameworkElement)?.Tag is UiTagRow row)
            {
                try { await _runner.WriteTagAsync(row.TagId, row.NewValue); Log($"Write OK TagId={row.TagId} Value='{row.NewValue}'"); }
                catch (Exception ex) { Log($"Write FAIL TagId={row.TagId}: {ex.Message}"); }
            }
        }
        private void Log(string msg) { DispatcherEx.UI(() => { TxtLog.AppendText($"[{DateTime.Now:HH:mm:ss}] {msg}"); TxtLog.ScrollToEnd(); }); }
    }
    ...
    public sealed class OpcDaPlcRunner : IDisposable
    {
        ...
        public async Task WriteTagAsync(long tagId, object value)
        {
            ...
            object typed;
            try { typed = CoerceToCanonicalVariant(it, Convert.ToString(value)); }
            catch (Exception ex) { throw new InvalidOperationException($"Valor inválido para tipo {((VarEnum)it.CanonicalDataType)}: {ex.Message}"); }

            Array shArr = Create1Based(new[] { sh });
            Array vlArr = Create1Based(new[] { typed });
            Array errors;

            _group.SyncWrite(1, ref shArr, ref vlArr, out errors);
            int err = Convert.ToInt32(errors.GetValue(1));
            if (err != 0) throw new InvalidOperationException($"OPC SyncWrite falhou. Tag='{_tagIdToItemId[tagId]}', VT={((VarEnum)it.CanonicalDataType)}, HRESULT=0x{err:X8}");
        ...
        private void Group_DataChange(int TxnId, int NumItems, ref Array ClientHandles, ref Array ItemValues, ref Array Qualities, ref Array TimeStamps)
        {
            ...
            for (int i = 1; i <= NumItems; i++)
            {
                ...
                _qMeasures.Enqueue(Tuple.Create(tagId, value, now)); OnValue?.Invoke(tagId, value, now);
            }
        }
        ...
        private async Task FlushMeasuresLoopAsync(CancellationToken ct)
        {
            ...
            while (!ct.IsCancellationRequested)
            {
                ...
                catch (Exception ex) { OnLog("FlushMeasuresLoopAsync: " + ex.Message); }
            }
        }
        ...
        private async Task PollWritesLoopAsync(CancellationToken ct)
        {
            ...
            while (!ct.IsCancellationRequested)
            {
                ...
                {
                    OnLog("PollWritesLoopAsync: " + ex.Message); try { EnsureDisconnected(); } catch { }
                }
            }
        }
        ...
        private void LoadConfigurationAndTags()
        {
            using (var db = NewContext())
            {
                ...
                int rate = 0; try { rate = Convert.ToInt32(plcRoot.Rate); } catch { } _opcUpdateRate = (int)Math.Max(100, rate > 0 ? rate : 500);
